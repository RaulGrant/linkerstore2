...
function extractReviewCount(countStr) {
...
function extractBrand(title, specs) {
  // Primero intentar obtener de specifications
  if (specs && (specs['Fabricante'] || specs['Marca'])) {
...
// Lista de ASINs reales de nuestras imágenes existentes
const availableAsins = [
  'B01MXMJYNA', 'B075RQPQPG', 'B07KG8MZYR', 'B08G8TQZYZ', 'B09LV7T8PQ', 'B0BKG1NQKR', 'B0CGF1MNXX'
];
...
function processProductData() {
  try {
    ...
    const processedProducts = rawProducts.map((product, index) => {
      ...
      // Asignar ASIN disponible o generar uno nuevo
      let asin;
      if (index < availableAsins.length) {
        asin = availableAsins[index];
      } else {
      ...
      return {
        ...
        description: cleanText(product.description) || `${cleanText(product.title)} - Producto industrial de alta calidad para uso profesional.`,
        price: extractPrice(product.price),
        currency: 'MXN',
        image_url: `/images/products/${asin}_Principal.webp`,
...
export const realAmazonProducts: AmazonProduct[] = ${JSON.stringify(processedProducts, null, 2)};`;
    ...
    console.log('Marcas principales:', Object.entries(brands).sort((a, b) => b[1] - a[1]).slice(0, 5));
